<template>
    <transition name="rappels">
        <div class="rappels" v-if="open">

            <div class="rappels__body">
                <div class="rappels__body__content">
                    <p class="rappels__body__content__title canala-deck">Type de rappel</p>
                    <p class="rappels__body__content__sub-title">Choisissez le type de rappel pour votre bracelet</p>

                    <div class="rappels__bory__content__rappels">
                    
                        <div
                            class="rappels__bory__content__rappels__one-rappel hover"
                            v-on:click="sonor = !sonor"
                            v-bind:class="{'rappels__bory__content__rappels__one-rappel-active-sonor': sonor}"
                        >
                            ...
                            <p class="rappels__bory__content__rappels__one-rappel__p">Sonore</p>
                        </div>

                        <div
                            class="rappels__bory__content__rappels__one-rappel hover"
                            v-on:click="vibror = !vibror"
                            v-bind:class="{'rappels__bory__content__rappels__one-rappel-active-vibror': vibror}"
                        >
                            ...
                            <p class="rappels__bory__content__rappels__one-rappel__p">Vibreur</p>
                        </div>

                        <div
                            class="rappels__bory__content__rappels__one-rappel hover"
                            v-on:click="led = !led"
                            v-bind:class="{'rappels__bory__content__rappels__one-rappel-active-led': led}"
                        >
                            ...
                            <p class="rappels__bory__content__rappels__one-rappel__p">Lumineux</p>
                        </div>
                    </div>

                    <button class="rappels__body__button hover" v-on:click="putRappels">Modifier</button>
                    <button class="rappels__body__button-canceled hover" v-on:click="$emit('rappels', {open: false, sonor: sonor, led: led, vibror: vibror})">Annuler</button>
                </div>
            </div>
        </div>
    </transition>
...
<script>
    ...
    export default {
        ...
        watch: {
            'open': function(val) {
                if (val) {
                    this.sonor = this.rappels.sonor
                    this.vibror = this.rappels.vibror
                    this.led = this.rappels.led
                }
            }
        }
    }
...
<style scoped>
    ...
    .rappels-enter, .rappels-leave-to {
        opacity: 0;
    }

    .rappels-enter-active, .rappels-leave-active {
    ...
    .rappels__body__title,
    .rappels__body__content__title {
        font-size: 2vw;
        font-weight: 700;
        margin: 2% 0 0;
    }

    .rappels__body__title__sub-title,
    .rappels__body__content__sub-title {
        font-size: 1vw;
    }


    .rappels__body__button {
    ...
    .rappels__body__button:hover {
        opacity: 0.8;
        transition: all 0.2s ease-in;
    }

    .rappels__body__button-canceled {
    ...
    .rappels__bory__content__rappels {
        display: flex;
    ...
    .rappels__bory__content__rappels__one-rappel {
        padding: 5%;
        width: 20%;
    ...
    .rappels__bory__content__rappels__one-rappel:hover {
        border: 1px solid var(--blue);
        transition: all 0.2s ease-in;
    }

    .rappels__bory__content__rappels__one-rappel-active-sonor,
    .rappels__bory__content__rappels__one-rappel-active-led,
    .rappels__bory__content__rappels__one-rappel-active-vibror {
        border: 1px solid var(--blue);
        transition: all 0.2s ease-in;
    }

    .rappels__bory__content__rappels__one-rappel svg {
        width: 100%;
        height: 60px;
        margin: 5% 0;
    }
